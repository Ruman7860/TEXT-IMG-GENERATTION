...
const Result = () => {
  ...
  const {generateImage,credit,user,setShowLogin} = useApp();
  ...
  const handleSubmit = async (e) => {
    ...
    if (credit <= 0) {
      ...
      navigate('/buy'); // Navigate to the credits purchase page
      setLoading(false);
  ...
  return (
    <motion.form 
      ...
      className='flex flex-col min-h-[75vh] items-center justify-center'
    >
      <div>
      ...
      </div>
      {
        ...
        >
          <input 
            ...
            className='flex-1 bg-transparent outline-none ml-8 max-sm:w-20 placeholder:text-[#e0e0e0] font-medium'
          />
          <button 
            type='submit'
